...
function App() {
  ...
  return <div className="app"><header className="topbar"><a className="brand" href="/"><span className="brand-mark">✦</span>TalentHub</a><nav aria-label="Main navigation"><a href="/positions">{t('Positions')}</a>{canManage && <a href="/attributes">{t('Attributes')}</a>}{user && <a href="/profile">{t('Profile')}</a>}{isAdmin && <a href="/admin">{t('Admin')}</a>}</nav><form className="header-search" onSubmit={submitSearch}><input value={search} onChange={event => setSearch(event.target.value)} placeholder={t('Search...')} aria-label={t('Search')} /></form><div className="header-actions"><select aria-label="Language" value={language} onChange={event => preference('language', event.target.value)}><option value="en">EN</option><option value="uz">UZ</option></select><button className="icon-button" aria-label="Toggle theme" onClick={() => preference('theme', theme === 'dark' ? 'light' : 'dark')}>{theme === 'dark' ? '☀' : '☾'}</button>{user ? <><span className="user-name">{user.firstName}</span><button className="btn btn-sm btn-outline-primary" onClick={() => signOut().catch(() => undefined).finally(() => setUser(null))}>{t('Sign out')}</button></> : <a className="btn btn-sm btn-primary" href="/login">{t('Sign in')}</a>}</div></header><main className="main-content"><Page user={user} onAuth={setUser} /></main></div>
}
