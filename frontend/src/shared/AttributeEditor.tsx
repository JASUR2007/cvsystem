...
export default function AttributeEditor({ value, options = [], onChange, userId }: { value: AttributeValue; options?: { id: string; value: string }[]; onChange: (value: AttributeValue) => void; userId?: string }) {
  ...
  switch (value.type) {
    ...
    case 'Boolean': return <select className="form-select" value={value.booleanValue === null ? '' : String(value.booleanValue)} onChange={event => change('booleanValue', event.target.value === '' ? null : event.target.value === 'true')}><option value="">Not set</option><option value="true">Yes</option><option value="false">No</option></select>
    case 'Dropdown': return <select className="form-select" value={value.selectedOptionId ?? ''} onChange={event => change('selectedOptionId', event.target.value || null)}><option value="">Select...</option>{options.map(option => <option key={option.id} value={option.id}>{option.value}</option>)}</select>
