...
export async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    response = await fetch(`${base}/api${path}`, { credentials: 'include', cache: 'no-store', ...options, headers })
  } catch {
...
export type Profile = { id: string; firstName: string; lastName: string; email: string; location: string | null; photoObjectKey: string | null; language: string; theme: string; version: number; recruiterRequestStatus?: string | null; roles?: string[] }
